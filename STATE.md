...
- Migrations `20260916120000_add_dynamic_qr_codes` and `20260917120000_qr_code_designs` are applied; production uses the same Supabase database as `.env.local` (confirmed by scanning a test code through production).
...
- QR designs (2026-09-17): dot style (`square | rounded | dots | soft`), corner style (`square | rounded | circle`), optional corner colour, background (`white | transparent`) and an optional frame label (max 32 characters) on `QrCode`; defaults reproduce the original plain design. All drawing goes through `layoutQr` and `layoutToSvg` in `src/lib/qr-codes/design.ts`, so the dashboard preview, SVG and PDF (`src/lib/qr-codes/render.ts`, drawn with pdf-lib) always match. Error correction is always H; dots under the logo plate are left out. PNGs are made in the browser from the server SVG (`src/components/qr-codes/qr-downloads.tsx`) because server-side rasterising needs a native library (sharp took the site down before); `/api/qr/[id]?format=png` stays as a plain fallback. PDFs skip SVG logos (pdf-lib cannot embed them). Company brand presets fill dot colour (brand_primary), corner colour (brand_accent) and logo.
- Scan testing: all 576 combinations (3 palettes, 4 dot styles, 3 corner styles, logo on/off, frame on/off, both backgrounds, at 180px and 600px) decoded with zxing-wasm, as did the server SVG and PDF output and the browser PNG path.
...
- PDF downloads leave out SVG logos; use the SVG or PNG download for those.
